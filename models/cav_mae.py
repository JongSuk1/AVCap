...
class MultiFramePatchEmbed(nn.Module):
    def __init__(self, num_frames, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()

        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        num_patches = (img_size[1] // patch_size[1]) * (img_size[0] // patch_size[0]) * (num_frames // 2)
        self.img_size = img_size
        self.patch_size = patch_size
        self.num_patches = num_patches

        self.proj = nn.Conv3d(in_chans, embed_dim, 
                              kernel_size=(2, patch_size[1], patch_size[0]), 
                              stride=(2, patch_size[1],patch_size[0]))
...
class CAVMAEFT(nn.Module):
    def __init__(self, label_dim, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
                 ...
                 norm_layer=nn.LayerNorm, norm_pix_loss=False, tr_pos=True, num_frames=1):
        super().__init__()
        ...
        if self.num_frames == 1:
            ...
            self.patch_embed_v = MultiFramePatchEmbed(num_frames, img_size, patch_size, in_chans, embed_dim)
        else:
    ...
    def initialize_weights(self):
        if self.num_frames == 4:
            torch.nn.init.normal_(self.pos_embed_a, std=.02)
            torch.nn.init.normal_(self.pos_embed_v, std=.02)
        else:
